...
def train(config: dict[str, Any]) -> dict[str, Any]:
    ...
    monitor = ResourceMonitor(
        interval_seconds=float(config.get("monitoring", {}).get("resource_sample_seconds", 1.0))
    )
        ...
        try:
            for model_name in config["modeling"]["models"]:
                with mlflow_tracking.model_run(config, model_name):
                    metrics, model_path, model = _train_one(model_name, frames, config, paths)
            ...
            resource_path = paths["monitoring_dir"] / "training_resource_usage.csv"
            monitor.write_csv(resource_path)
            logger.info("Saved training resource usage to %s", resource_path)
            if mlflow_tracking.tracking_enabled(config):
                mlflow_tracking.log_artifacts([resource_path], artifact_path="monitoring")
